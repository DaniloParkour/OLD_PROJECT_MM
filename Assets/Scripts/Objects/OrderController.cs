...
public class OrderController : MonoBehaviour {
	...
	private Order currentoorderOnZoom;
	...
	void Update () {
		...
		if ((Input.GetMouseButtonUp (0)) && !GameSettings.panelOpened) {
			...
			if(hit.collider != null){
				...
				if(order != null){
					if(currentoorderOnZoom == null) {
						currentoorderOnZoom = order;
						if(!gameManager.chooseOrder(order))
							order.zoom();
					} else if (order == currentoorderOnZoom) {
						choosedOrder = currentoorderOnZoom;
						gameManager.chooseOrder(choosedOrder);

					} else {
						currentoorderOnZoom.backZoom();
						currentoorderOnZoom = null;
					}
				}
			} else if(currentoorderOnZoom != null){
				currentoorderOnZoom.backZoom();
				currentoorderOnZoom = null;
			}
		}
	...
	public void initOrders(int value){
		...
		gameManager.activePanelScore (0.5f, false);
	...
	public void removeOrders () {
		...
		onInitOrders = false;
		gameManager.activePanelScore (1, true);
	}
